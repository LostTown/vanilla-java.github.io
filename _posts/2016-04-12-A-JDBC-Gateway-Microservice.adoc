...
The service supports two messages `executeQuery` and `executeUpdate`.  These methods mirror the same methods for `PrepatedStatement` except the results are passed as messages
